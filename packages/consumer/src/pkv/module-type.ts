	...
	function analyzeThing(value: any, path: string): void {
		if (value && typeof value === 'object') {
			if (Array.isArray(value)) {
				...
				let explicit = false;
				const conditionImport = Boolean(
					'import' in record && record.import,
				);
				const conditionRequire = Boolean(
					'require' in record && record.require,
				);
				const conditionDefault = Boolean(
					'default' in record && record.default,
				);
